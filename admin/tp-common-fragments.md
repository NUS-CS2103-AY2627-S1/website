...
Caution: This is <span class="text-danger">very hard to rectify later</span>, after the PR containing the commits has been merged. %%Reason: While Git allows editing past commits, doing so changes their timestamps, which affects your weekly code contribution stats (which are factored into evaluating the consistency of your coding work over the project duration).%% { icon="fas-skull" i-class="text-danger" }
{% endcall %}
