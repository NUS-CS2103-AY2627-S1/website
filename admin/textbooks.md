...
**Topics relevant for a week appear under the weekly schedule page** (see the <span class="text-primary">{{ show_as_tab('Topics', icon_tab_topics) }}</span> tab). Some of those topics come with pre-recorded videos. ==Watching those videos is encouraged but optional==: while they cover no theory beyond the text version, they can offer complementary explanations, more examples, visualizations, etc.
</div>
...
* ==We recommend watching <span class="badge rounded-pill bg-danger">++{{ icon_video }} Videos++</span> provided at faster speeds== (`x1.25` or even `x1.5`) _if_ the current pace of the video feels 'too slow' for you.{% if cs2113 %}
